    ...
    <div class="text-center space-y-6">
        ...
        <div x-show="!verified" class="space-y-4">
            <div class="p-4 bg-gray-50 border border-gray-200 rounded-lg text-left">
                <p class="text-xs font-semibold text-gray-700 dark:text-gray-200 mb-2">What to do next:</p>
            ...
            <button 
                ...
                :disabled="checking"
                class="w-full px-6 py-2.5 text-sm text-gray-700 dark:text-gray-200 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors font-medium disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
        ...
        <div class="pt-4 border-t border-gray-200">
            @auth
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="text-sm text-gray-600 dark:text-gray-300 hover:text-gray-900 transition-colors">
                        Logout
                    </button>
                </form>
            @else
                <a href="{{ route('login') }}" class="text-sm text-gray-600 dark:text-gray-300 hover:text-gray-900 transition-colors">
                    Back to Login
                </a>
            @endauth
        </div>
