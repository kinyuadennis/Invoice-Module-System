...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
    <div style="background-color: #f8f9fa; padding: 20px; border-radius: 8px; margin-bottom: 20px;">
        <h1 style="color: #2B6EF6; margin-top: 0;">Estimate #{{ $estimateNumber }}</h1>
    ...
    <div style="background-color: #fff; padding: 20px; border: 1px solid #e0e0e0; border-radius: 8px; margin-bottom: 20px;">
        ...
        <p>We hope this email finds you well. Please find attached estimate #{{ $estimateNumber }} for your review.</p>

        <div style="background-color: #f8f9fa; padding: 15px; border-radius: 6px; margin: 20px 0;">
            <table style="width: 100%; border-collapse: collapse;">
                <tr>
                    <td style="padding: 8px 0; font-weight: bold;">Estimate Number:</td>
                    <td style="padding: 8px 0; text-align: right;">{{ $estimateNumber }}</td>
                </tr>
                <tr>
                    <td style="padding: 8px 0; font-weight: bold;">Issue Date:</td>
                    <td style="padding: 8px 0; text-align: right;">{{ $estimate->issue_date ? \Carbon\Carbon::parse($estimate->issue_date)->format('F d, Y') : 'N/A' }}</td>
                </tr>
                @if($expiryDate)
                <tr>
                    <td style="padding: 8px 0; font-weight: bold;">Expiry Date:</td>
                    <td style="padding: 8px 0; text-align: right;">{{ \Carbon\Carbon::parse($expiryDate)->format('F d, Y') }}</td>
                </tr>
                @endif
        ...
        <p style="color: #666; font-size: 14px;">
            <strong>This estimate is valid until:</strong> {{ \Carbon\Carbon::parse($expiryDate)->format('F d, Y') }}
        </p>
        @endif
